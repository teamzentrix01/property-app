...
import { sendDocumentVerifiedEmail, sendDocumentRejectedEmail } from "@/lib/mailer";
...
export async function PATCH(req, { params }) {
  ...
  if (normalized) {
    ...
    const targetUser = await prisma.user.findUnique({ where: { id: normalized.userId }, select: { id: true, name: true, email: true } });
    if (targetUser?.email) {
      ...
      if (body.status === "VERIFIED") {
        await sendDocumentVerifiedEmail({
          ...
          allVerified: becameActive,
          origin,
        }).catch((err) => console.error("Document verified email failed:", err));
        ...
        await sendDocumentRejectedEmail({
          ...
          origin,
        }).catch((err) => console.error("Document rejected email failed:", err));
      }
    }
  ...
  if (body.status === "VERIFIED") {
    ...
    if (statuses.aadhaarStatus === "VERIFIED" && statuses.panStatus === "VERIFIED" && statuses.voterIdStatus === "VERIFIED") {
      Object.assign(data, { verificationStatus: "ACTIVE", verifiedAt: now, rejectedAt: null, rejectionReason: null });
      allVerifiedLegacy = true;
    }
  }
  const updated = await prisma.user.update({ where: { id }, data, select: { id: true, name: true, email: true, verificationStatus: true, aadhaarStatus: true, panStatus: true, voterIdStatus: true } });
  ...
  if (user?.email) {
    ...
    if (body.status === "VERIFIED") {
      await sendDocumentVerifiedEmail({
        user,
        documentType: body.document,
        allVerified: allVerifiedLegacy,
        origin,
      }).catch((err) => console.error("Legacy document verified email failed:", err));
    } else if (body.status === "REJECTED") {
      await sendDocumentRejectedEmail({
        user,
        documentType: body.document,
        reason,
        origin,
      }).catch((err) => console.error("Legacy document rejected email failed:", err));
    }
  }

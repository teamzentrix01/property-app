...
export default function PropertyGallery({ photos = [], title }) {
  ...
  if(!photos.length)return <div className="grid h-72 place-items-center rounded-2xl bg-paper-dim text-sm text-ink-soft md:h-[460px]">No property photos available</div>;
  const count=photos.length;
  return <><div className={`relative grid overflow-hidden rounded-2xl bg-paper-dim ${count===1?"grid-cols-1":count===2?"grid-cols-[1.35fr_1fr] gap-1":"grid-cols-1 gap-1 md:grid-cols-2"}`}>

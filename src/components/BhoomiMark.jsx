export default function BhoomiMark({ compact = false, light = false }) {
  return (
    <div className="flex items-center gap-2.5">
      <span className="grid h-9 w-9 place-items-center rounded-xl bg-moss text-white shadow-sm" aria-hidden="true">
      ...
      {!compact && <span className={`font-display text-2xl font-bold tracking-tight ${light ? "text-paper" : "text-ink"}`}>Bhoomi</span>}
    </div>
  );
}
